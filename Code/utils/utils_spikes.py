...
class dummy_spikestructure:
    ...
    def __init__(self, t, i, sinusoid=[]):
        '''
    ...
    def plot_isilong_vs_intrinsicfreqandinputfrequency(self,neurons_n, frequencies, s_osc_intrinsic, norm = False, cmap = None, min_max = None, img_title = None):
        ...
        yticks([i for i in range(neurons_n)], ticks)
        colorbar()
    def plot_isi_vs_intrinsicfreqandinputfrequency(self, neurons_n, frequencies, s_osc_intrinsic, norm = False, cmap = None, min_max = None, img_title = None):
        if cmap == None:
            cmap = 'Purples'
        freq = 0
        neuron = 0
    ...
    def divide_indexes(self, t, i, sinusoid=[]):
        ...
        # array = np.unique(i)
        self.dummyspike_collection = []
        for idx,i_singular in enumerate(range(int(max_i))):
...
def generate_spikes_from_freq(given_frequency, duration=1, break_time=0.1, time_array=None, neuron_array=None):
    ...
    for ix in range(len(given_frequency)):
        ...
        neuron_array = np.append(neuron_array, neuron_array_here)

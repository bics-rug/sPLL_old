...
class dummy_spikestructure:
    ...
    def __init__(self, t, i, sinusoid=[], verbose=True):
        '''
        ...
        if self.ismultipleidx():
            ...
            for d_ix, dummy_structure in enumerate(self.dummyspike_collection):
                if d_ix in neuron_list:
                    dummy_structure.plot_instfreq(average=average, offset=offset + idx, gain=gain, norm=norm,
                                                  label=label,
    ...
    def plot_isilong_vs_intrinsicfreqandinputfrequency(self, neurons_n, frequencies, s_osc_intrinsic, norm=False,
    ...
    def plot_isishort_vs_intrinsicfreqandinputfrequency(self, neurons_n, frequencies, s_osc_intrinsic, norm=False,
                                                        cmap=None, min_max=None, img_title=None):
        if cmap == None:
            cmap = 'Purples'
        freq = 0
    ...
    def plot_isi_vs_intrinsicfreqandinputfrequency(self, neurons_n, frequencies, s_osc_intrinsic, norm=False, cmap=None,
                                                   min_max=None, img_title=None, show=True):
        if cmap == None:
            cmap = 'Purples'
        freq = 0
        neuron = 0
...
def generate_spikes_from_freq(given_frequency, duration=1, break_time=0.1, time_array=None, neuron_array=None,
                              trials_per_stimulus=1, noise=False):
    ...
    if noise == True:
        noise = np.random.normal(-1 / (5 * 0.1), 1 / (5 * 0.1), len(time_array))
        noise[np.abs(noise) < 100e-6] = 100e-6
        time_array_noisy_neg = time_array + noise

        time_array_noisy = time_array_noisy_neg[time_array_noisy_neg > 0]
        time_array_noisy = np.round(time_array_noisy,5)
        neuron_array_noisy = neuron_array[time_array_noisy_neg > 0]
        neuron_array_noisy_nodouble = np.array([])
        time_array_noisy_nodouble = np.array([])
        for ix in range(len(given_frequency)):
            for trial in range(trials_per_stimulus):
                time_array_noisy_idx = time_array_noisy[neuron_array_noisy == ix * trials_per_stimulus + trial]
                _,where_unique = np.unique(time_array_noisy_idx, return_index = True)
                time_array_noisy_idx = time_array_noisy_idx[where_unique]
                neuron_array_noisy_nodouble = np.append(neuron_array_noisy_nodouble,[ix * trials_per_stimulus + trial for j in range(len(time_array_noisy_idx))])
                time_array_noisy_nodouble = np.append(time_array_noisy_nodouble,time_array_noisy_idx)
        # print(noise)

        # shift = 1e-3 * dt
        # uuu = np.argsort(time_array_noisy_nodouble)
        # time_array_noisy_nodouble = time_array_noisy_nodouble[uuu]
        # neuron_array_noisy_nodouble = neuron_array_noisy_nodouble[uuu]
        # timebins = np.asarray(np.asarray(time_array_noisy_nodouble + shift) / dt,
        #                       dtype=np.int32)
        # eee = np.where(np.logical_and(np.diff(timebins) == 0, np.diff(neuron_array_noisy_nodouble) == 0))
        return neuron_array_noisy_nodouble, time_array_noisy_nodouble, simulation_array
    else:

...
## SCRIPT BEGINS
code_name = 'PLL_test'
# 1 ==================== Set up the script folders and importing custom libraries ====================
...
def create_PLL(input_n,neurons_n, neuron_array, time_array):
    ...
    TDE_model = Equations('''
                             dv/dt = -v/(tau_TDE) +Ie_NMDA_h/(0.01*pF) : volt
                             dIe_h/dt = -Ie_h/(tau_TDE_Ie_h) : amp
                             dIe_NMDA_h/dt = -Ie_NMDA_h/(tau_TDE_Ie_NMDA_h) : amp
                             ''')
    TDE_Neuron_plus = NeuronGroup(neurons_n*input_n, model=TDE_model, method='euler', threshold='v > v_TDE_threshold',
    ...
    HP_model = Equations('''
                                        dv/dt = -v/(tau_HP) + (I + Ie - Ii)/(0.01*pF): volt
                                        I : amp
                                        dIe/dt = -Ie/(tau_HP_Ie) : amp
                                        dIi/dt = -Ii/(tau_HP_Ii) : amp
                                        ''')

    HP0 = NeuronGroup(neurons_n*input_n, model=HP_model, method='euler', threshold='v > v_HP_threshold',
                      refractory='refrac_HP ',
                      reset='v = 0*mV')
    HP1 = NeuronGroup(neurons_n*input_n, model=HP_model, method='euler', threshold='v > v_HP_threshold', refractory='refrac_HP',
                      reset='v = 0*mV')
    HP2 = NeuronGroup(neurons_n*input_n, model=HP_model, method='euler', threshold='v > v_HP_threshold', refractory='refrac_HP',
                      reset='v = 0*mV')
    WTA_model = Equations('''
                                        dv/dt = -v/(tau_WTA) + (I + Ie - Ii)/(0.01*pF) : volt
                                        I : amp
    ...
    nWTA = NeuronGroup(neurons_n*input_n, model=WTA_model, method='euler', threshold='v > v_WTA_threshold',
                       refractory='refrac_WTA',
                       reset='v = 0*mV')
    nWTA.I = 'WTA_stable_current'
    Poisson_to_nWTA = PoissonGroup(input_n, 200 * Hz)
    gWTA = NeuronGroup(input_n, model=WTA_model, method='euler', threshold='v > v_WTA_threshold', refractory='refrac_WTA',
                       reset='v = 0*mV')

    return Fake_Neuron, Oscillator_neuron, TDE_Neuron_plus, HP0, HP1, HP2, nWTA, gWTA, Poisson_to_nWTA
...
def create_synapses(Fake_Neuron, Oscillator_neuron, TDE, HP0, HP1, HP2, nWTA, gWTA, Poisson_to_nWTA):
  ...
  # set up connection for "trigger"
    ...
    return NMDA_h_plus,S_h_plus,FB_syn,HP0_syn,HP1_syn,HP2_syn,to_nWTA,from_gWTA,to_gWTA, Poisson_syn, normalize_WTA
def create_monitors(Fake_Neuron, Oscillator_neuron, nWTA, HP0, HP1, HP2, TDE, gWTA):
    ...
    return Fake_spikes, Osc_spikes, nWTA_spikes, HP0_spikes, HP1_spikes, HP2_spikes, TDE_spikes, Gwta_spikes, TDE_spikes
def run_the_script(data):
    index = data[0]

    parameters = data[1]
    # print("The trasmitted parameters are " + str(parameters['spatial_frequency']))
...
for i in range(len(parameters['frequencies'])):
    stimuli += [i for j in range(parameters['neurons_n'])]
